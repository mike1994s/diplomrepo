...
function addWinners(vkids, type, tokens, vkID, gameId, file){
	type = type || _VK_TYPE;
	...
	promise.then(function(winners) {
		...
		sendNotification(tokens, vkID, gameId, file, answer);
		return answer;
	})
	.catch(function(err){
		console.log(err);
...
//addWinners(vkids, type, tokens, vkID, gameId, file)
		addWinners(arrInvited, _VK_TYPE, arr, user, gameId, file);
		return "ok";
	});
}
